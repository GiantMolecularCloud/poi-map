...
class POIData(pa.DataFrameModel):
    latitude: Series[float] = pa.Field(
        ge=-90.0, le=90.0, title="Latitude", description="Latitude of the POI."
    )
    longitude: Series[float] = pa.Field(
        ge=-180.0, le=180.0, title="Longitude", description="Longitude of the POI."
    )
    ...
    def category_must_be_list(cls, series: Series) -> Series[bool]:
        ...
        return series.apply(
            lambda x: isinstance(x, np.ndarray) and all(isinstance(i, str) for i in x)
        )
